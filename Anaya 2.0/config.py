...
class LLMConfig:
    ...
    temperature: float = field(default_factory=lambda: float(os.getenv("OLLAMA_TEMPERATURE", "0.82")))
    top_p: float = field(default_factory=lambda: float(os.getenv("OLLAMA_TOP_P", "0.9")))
    repeat_penalty: float = field(default_factory=lambda: float(os.getenv("OLLAMA_REPEAT_PENALTY", "1.18")))
    presence_penalty: float = field(default_factory=lambda: float(os.getenv("OLLAMA_PRESENCE_PENALTY", "0.15")))
    stream_output: bool = field(default_factory=lambda: os.getenv("OLLAMA_STREAM_OUTPUT", "True").lower() == "true")
    context_window_turns: int = field(default_factory=lambda: int(os.getenv("OLLAMA_CONTEXT_TURNS", "16")))
    max_facts_in_prompt: int = field(default_factory=lambda: int(os.getenv("OLLAMA_MAX_FACTS", "6")))
    num_ctx: int = field(default_factory=lambda: int(os.getenv("OLLAMA_NUM_CTX", "4096")))
